...
- **Learning note** (`src/content/posts/` with the `learning-notes` tag): a concise personal record of something TC read or dug into, not a teaching piece. Distinct from a pillar post in four ways. (1) Density: note-cadence, fragments allowed ("Not caching inefficiently. Not caching at all."); record the valuable facts and numbers and cut setup, hedges, and narration. (2) Headers: plain topic-or-finding labels only — never storytelling headers. `MXFP4: multiply without dequantizing` and `Expert cache: allocation beats capacity` are headers; `The trick I didn't expect` and `The bug that doesn't crash` are stories, so rename them. (3) **Ground every term of art in a concrete example before reasoning about it** — see the rule below. (4) Structure: keep `description`, `tldr`, a closing **Key takeaways** list, and primary-source links; drop the `faq` block and the answer-capsule polish. The stance is TC learning in public, first person, not TC explaining to beginners. Reference example: `2026-08-03-reading-kimi-k3-in-c.md`.

  **The grounding rule (learning notes).** Density is not an excuse to skip the concrete. A learning note has to stand alone: a reader who does not already know the domain should be able to follow the decisions and trade-offs without opening the source. So before reasoning about any term of art, show it.
  - **Name the setup in physical terms.** Not "a multiagent system" but "80 instances of one model, separate context windows, one shared git repo and one forum, 12-hour session." Not "an agent harness" but "the loop that calls the model, runs its tool calls, feeds results back — plus the permission prompt and the compaction."
  - **Define by worked example, not by definition.** For each named concept, give the smallest scenario that makes the failure visible: two agents both decide the game needs a game loop, and nothing in either context says the other exists.
  - **Mine the source's references, not just its prose.** Papers and engineering posts often discuss mechanisms and leave the concrete example in the work they cite. A "hidden profile" task or a "Bertrand pricing game" has an original paper with a real experimental setup, and that setup is the example worth describing. Chase the citation, and cite it — this also satisfies the citation-density standard in §10.
  - **This is grounding, not tutorial.** One short paragraph or a table per concept, placed before the finding that depends on it. If it runs longer than the finding it supports, it has become a teaching piece; cut it back.
