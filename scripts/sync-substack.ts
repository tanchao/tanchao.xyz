...
async function fetchWithRetry(url: string): Promise<Response | null> {
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (res.status === 403 || res.status === 401) {
      // Substack sits behind Cloudflare bot management. Data-center IPs
      // (GH Actions, CF Workers, etc.) get blocked regardless of headers
      // or cookies. Run this script from a residential IP. Fail loudly
      // so the local launchd job surfaces the block instead of silently
      // reporting "Imported 0 notes".
      console.error(
        `Substack API returned ${res.status}. The IP is likely blocked by Cloudflare bot management. Run this script from a residential IP (your Mac on home Wi-Fi, not a data-center).`,
      );
      process.exit(1);
    }
