...
async function fetchWithRetry(url: string): Promise<Response | null> {
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    // Substack sits behind Cloudflare bot management which blocks
    // data-center IPs (GH Actions, CF Workers, etc.) regardless of
    // headers or cookies. Run this from a residential IP. Hard-failing
    // on 401/403 surfaces the block instead of pretending success.
